...
public interface InterestRequestRepository extends JpaRepository<InterestRequest, Long> {
	...
	@Modifying
	@Transactional
	@Query("DELETE FROM InterestRequest r WHERE r.senderProfile.id = :profileId")
	void deleteAllSentRequests(@Param("profileId") Long profileId);

	@Modifying
	@Transactional
	@Query("DELETE FROM InterestRequest r WHERE r.receiverProfile.id = :profileId")
	void deleteAllReceivedRequests(@Param("profileId") Long profileId);
}
