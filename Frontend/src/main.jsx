...
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL ?? "https://soulsync-ai-8hyt.onrender.com/api";
...
function App() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/profiles/${loggedInProfileId}`,
      {
        method: "DELETE",
      }
    );
    ...
    // Remove deleted profile from UI
    setProfiles((prev) =>
      prev.filter((p) => p.id !== Number(loggedInProfileId))
    );
...
if (selectedImage) {
  ...
  try {
    ...
    setProfiles((prevProfiles) =>
      prevProfiles.map((p) =>
        p.id === savedProfile.id
          ? withPhotos([savedProfile])[0]
          : p
      )
    );
    ...
    <main className="login-page">
      ...
      <section className="login-panel">
        <div className="login-panel-header">
          <p className="eyebrow">SoulSync AI private beta</p>
          <h1>
            {mode === "login"
      ...
      >
        ...
        {showMenu && (
          <div className="profile-dropdown">
            ...
            <img src={profile.photo} alt="" className="dropdown-avatar" />
...
function Dashboard({ profile, matches, status, onOpenProfile, onReport, onLogout }) {
